...
> A release-by-release plan for adding **local, on-device AI** (via WebLLM) to StudyTrak without breaking its single-file, no-build, no-server character.
...
| Tier | Model | Approx download | Use cases |
|---|---|---|---|
| **Tiny** | Qwen2.5-0.5B-Instruct | ~400 MB | Parsing, classification, quick-add |
| **Small** | Phi-3.5-mini (3.8B, q4) | ~2.3 GB | Briefings, planning, tutor chat |
| **Vision (later)** | Moondream2 | ~1.8 GB | Photo → assignment |
| **Embedding** | all-MiniLM-L6-v2 (Transformers.js) | ~25 MB | Tutor retrieval |
...
| # | Feature | Desirability | Novelty | Feasibility | Privacy fit | Local-model fit | Composite |
|---|---|---:|---:|---:|---:|---:|---:|
| 5.1 | Photo → assignment (Moondream2 in browser) | 9 | 9 | 4 | 10 | 6 | **7.6** |
| 5.2 | Photo → timetable import | 8 | 8 | 4 | 10 | 6 | **7.2** |
| 5.3 | Handwriting-to-text for review notes | 7 | 8 | 3 | 10 | 5 | **6.6** |
| 5.4 | Whiteboard OCR mode (live camera tab) | 6 | 9 | 2 | 10 | 5 | **6.4** |

**Release goal:** Eliminate typing for the highest-friction inputs. Stretch release because vision models stress download size and lower-end hardware.
...
2. **Quality of small models on planning tasks.** A 0.5B model can parse "math hw due Friday" but probably can't write a good weekly plan. Likely we land on Tiny for R1 and Small for R2+, with the user choosing.
